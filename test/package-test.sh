...
main() {
	jq -e '.manifestVersion == 2 and .version == "2.0.12" and .upstreamVersion == "0.77.1" and .minBoxVersion == "9.1.0" and .iconUrl != "" and .packagerName != "" and .packagerUrl == "https://github.com/marcusquinn" and (has("packageUrl") | not) and (.mediaLinks | length) > 0 and .changelog == "file://CHANGELOG"' \
	...
	assert_contains CHANGELOG '[2.0.12]' || return 1
	assert_contains CHANGELOG.md '[2.0.12] - 2026-09-11' || return 1
	assert_contains SECURITY.md '| 2.0.12      | 0.77.1           | Yes        |' || return 1
